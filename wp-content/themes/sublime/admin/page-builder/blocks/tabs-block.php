<?php
/* Tabs Block - extends aqua page builder tabs block to include vertical tabs */
if(!class_exists('PG_Tabs_Block')) {
	class PG_Tabs_Block extends AQ_Block {
		...
		function __construct() {
			...
			//create the widget
			parent::__construct('pg_tabs_block', $block_options);
			...
			if($type == 'tab') {
				...
				$output .= '<div class="tabbable tabs-left">';
					...
					foreach( $tabs as $tab ){
						...
						$output .= '<li class="'.$tab_selected.'"><a href="#aq-tab-'. sanitize_title( $tab['title'] ) . $i .'" data-toggle="tab">' . $tab['title'] . '</a></li>';
						$i++;
					}
					...
					$i = 1;
					foreach($tabs as $tab) {
						$tab_pane_selected = $i == 1 ? 'active' : '';
						$output .= '<div id="aq-tab-'. sanitize_title( $tab['title'] ) . $i .'" class="tab-pane '.$tab_pane_selected.'">'. wpautop(do_shortcode(htmlspecialchars_decode($tab['content']))) .'</div>';
